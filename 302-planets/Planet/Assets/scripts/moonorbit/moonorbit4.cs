...
public class moonorbit4 : MonoBehaviour
{
    ...
    private Vector3 FindOrbitPoint(float angle, float mag)
    {
        ...
        pos.z += Mathf.Cos(angle) * mag * .5f - orbitCenter.position.z;
        pos.y += Mathf.Sin(angle) * mag - orbitCenter.position.y;
        pos.x += Mathf.Sin(angle) * mag * .4f - orbitCenter.position.x;
