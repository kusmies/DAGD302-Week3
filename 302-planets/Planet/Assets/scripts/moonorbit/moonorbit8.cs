...
public class moonorbit8 : MonoBehaviour
{
    ...
    private Vector3 FindOrbitPoint(float angle, float mag)
    {
        ...
        pos.z += Mathf.Sin(angle) * mag * .3f - orbitCenter.position.z;
        pos.y += Mathf.Cos(angle) * mag * -.8f - orbitCenter.position.y;
        pos.x += Mathf.Sin(angle) * mag * .2f - orbitCenter.position.x;
