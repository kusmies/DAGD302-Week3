...
public class moonorbit6 : MonoBehaviour
{
    ...
    private Vector3 FindOrbitPoint(float angle, float mag)
    {
        ...
        pos.z += Mathf.Sin(angle) * mag * -.8f - orbitCenter.position.z;
        pos.y += Mathf.Sin(angle) * mag * .9f - orbitCenter.position.y;
        pos.x += Mathf.Cos(angle) * mag - orbitCenter.position.x;
